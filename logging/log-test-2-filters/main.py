...
def run():
    ...
    external= logging.getLogger("external_library")

    external.error("Lamia: %s", { "test": "data" })
    external.warning("Lamia debug: %s", { "test": "data" })
    external.info("Lamia info: %s", { "test": "data" })

    app_logger.info("Info message in app logger, data: %s", { "test": "data" })
    views_logger.warning("Debug message in views logger, data: %s", { "test": "data" })
